...
# @test_steps('Update list', 'Get list request')
#
...
@pytest.mark.parametrize('list_id, response_status', [
    ('901417264550', 200),
    ('3453453', 401),
    ('sajkdakjsd', 400)
])

def test_get_list_request(list_id, response_status):
    response = requests.get('https://api.clickup.com/api/v2/list/' + list_id, headers=headers_variable)
    # print('Check a Status Code response_for_get_list: ', response.status_code)
    assert response.status_code == response_status

# def test_get_list_request_neg():
#     response = requests.get('https://api.clickup.com/api/v2/list/901417264550', headers=headers_variable)
#     # print('Check a Status Code response_for_get_list: ', response.status_code)
#     assert response.status_code == 200